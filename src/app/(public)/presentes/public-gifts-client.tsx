  ...
  return (
    <div className="flex flex-col gap-8 pb-16">
      ...
      <Dialog open={!!selectedGift} onOpenChange={(open) => !open && setSelectedGift(null)}>
        <DialogContent className="max-w-2xl p-0 overflow-hidden rounded-3xl border-zinc-200 shadow-2xl">
          {selectedGift && (
            <div className="flex flex-col md:flex-row">
              {/* Lado Esquerdo: Imagem do Presente */}
              <div className="md:w-1/2 bg-zinc-100 relative min-h-[260px] md:min-h-full flex items-center justify-center overflow-hidden">
                {selectedGift.imageUrl ? (
                  ...
                  <div className="flex flex-col items-center justify-center text-zinc-400 p-8 text-center">
                    <GiftIcon className="w-12 h-12 text-zinc-300 mb-2 stroke-[1.5]" />
                    <span className="text-xs text-zinc-500 font-semibold">Lembrança para o Novo Lar</span>
                  </div>
                )}

                {selectedGift.isPurchased && (
                  <div className="absolute inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4 text-center">
              ...
              <div className="md:w-1/2 p-6 md:p-8 flex flex-col justify-between space-y-6 bg-white">
                <div>
                  ...
                  <h2 className="text-2xl font-bold text-zinc-900 tracking-tight font-serif italic">
                    {selectedGift.title}
                  </h2>
                  ...
                  <div className="mt-6 bg-zinc-50 p-4 rounded-2xl border border-zinc-100 space-y-1">
                    <span className="text-[11px] text-zinc-400 uppercase tracking-wider font-bold block">
                    ...
                    <p className="text-[11px] text-zinc-500 flex items-center gap-1 mt-1">
                      <Sparkles className="w-3 h-3 text-amber-600" /> Pagamento via PIX ou Cartão de Crédito
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-3 pt-2">
                  {selectedGift.isPurchased ? (
                    <Button disabled className="w-full rounded-2xl py-6 text-sm font-bold bg-zinc-100 text-zinc-400 cursor-not-allowed">
                      Presente Já Comprado
                    </Button>
                  ) : (
                    <Link href={`/checkout/${selectedGift.id}`} className="block w-full">
                      <Button className="w-full bg-gradient-to-r from-amber-700 via-amber-800 to-zinc-900 hover:from-amber-800 hover:to-zinc-800 text-white rounded-2xl py-6 text-base font-bold shadow-lg shadow-amber-900/10 flex items-center justify-center gap-2">
                  ...
                  <Button
                    ...
                    className="w-full text-xs text-zinc-500 hover:text-zinc-900 rounded-xl"
                  >
                    Voltar para a lista
                  </Button>
                </div>
